...
app.get('/api/flyer-templates', (req, res) => {
  fs.readFile(path.join(__dirname, './json/flyer-templates.json'), 'utf8', (err, jsonString) => {
    if (err) {
      console.error(err);
      res.status(500).send({ error: 'Could not load flyer templates' });
      return;
    }
...
// Generate flyer (HTML with param replacement)
app.post('/api/generate-flyer', (req, res) => {
  const { templateId, params } = req.body;
  if (!templateId || !params) {
    res.status(400).send({ error: 'templateId and params are required' });
    return;
  }
  fs.readFile(path.join(__dirname, './json/flyer-templates.json'), 'utf8', (err, jsonString) => {
    if (err) {
      res.status(500).send({ error: 'Could not load templates' });
      return;
    }
    const templates = JSON.parse(jsonString);
    const template = templates.find(t => t.id === templateId) || (templates.data && templates.data.find(t => t.id === templateId));
    if (!template) {
      res.status(404).send({ error: 'Template not found' });
      return;
    }
    const layoutFile = path.join(__dirname, 'public', template.layout);
    fs.readFile(layoutFile, 'utf8', (err, html) => {
      if (err) {
        res.status(500).send({ error: 'Could not load template layout' });
        return;
      }
      let result = html;
